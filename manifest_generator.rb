...
#options[:deployment_address] = ENV["DEPLOYMENT_ADDRESS"] + ENV["SVN_REVISION"]
#workspace = ENV["WORKSPACE"]
#options[:ipa_path] = workspace.to_s + "/build/"
...
ipa_file_name = nil
Dir.glob("#{options[:ipa_path]}/*.ipa") do |filename|
  ipa_file_name = File.basename(filename)
  break
end

if (ipa_file_name == nil)
  raise ArgumentError, "IPA file not found"
end

ipaPath = "#{options[:ipa_path]}/#{ipa_file_name}"

info_plist_content = nil
Zip::ZipFile.open(ipaPath) do |zip_file|
  zip_file.each do |f|
  		...
  		<dict>
  			...
  			<array>
  				<dict>
  					...
  					<key>url</key>
  					<string>#{options[:deployment_address]}/#{ipa_file_name}</string>
  				</dict>
  			</array>
  			<key>metadata</key>
  			<dict>
...
puts "Manifest generated to #{output_file_name}"