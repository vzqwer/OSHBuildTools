...
Usage: #{executable_name} [options] output_file_name
  ...
  opts.on('-f MANIFEST_NAME', 'Manifest plist file name') do |manifest_name|
    options[:manifest_name] = manifest_name
  end
end
...
if (options[:app_name] == nil)
  raise ArgumentError, "You must specify an app name with -n APPLICATION_NAME"
end

if (options[:manifest_name] == nil)
  raise ArgumentError, "You must specify the full manifest path with -f MANIFEST_NAME"
end

if (options[:manifest_path] == nil)
  raise ArgumentError, "You must specify the full manifest plist file name with -m MANIFEST_PATH"
end
...
File.open(options[:template_file], 'r') do |template_file|
  File.open("#{output_file_name}",'w') do |output_file|
    template_file.readlines.each do |template_line|
      ...
      output_line = output_line.sub("\[MANIFEST_PATH\]", "#{options[:manifest_path]}/#{options[:manifest_name]}")
